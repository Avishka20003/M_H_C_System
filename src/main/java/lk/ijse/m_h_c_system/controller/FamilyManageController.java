...
import lk.ijse.m_h_c_system.dto.*;
import lk.ijse.m_h_c_system.dto.tm.ChildTm;
import lk.ijse.m_h_c_system.dto.tm.ClincTm;
import lk.ijse.m_h_c_system.dto.tm.FatherTm;
import lk.ijse.m_h_c_system.dto.tm.MotherTm;
import lk.ijse.m_h_c_system.model.ChildModel;
import lk.ijse.m_h_c_system.model.FatherModel;
import lk.ijse.m_h_c_system.model.MotherModel;
...
// Child table section Start
    ...
    private void loadFamilyCards(double gridWidth) {
        gridFamily.getChildren().clear(); // Clear old cards
    ...
    public void btnAddMotherOnAction(ActionEvent actionEvent) {
        try {
            ...
            MotherDto dto = new MotherDto(name, address, nic, dob, occupation, email, phoneNumber, weight, height, allergies, medicalHistory, note, blood);
    ...
    public void loadMotherTableData() throws SQLException, ClassNotFoundException {
        tblMother.setItems(FXCollections.observableArrayList(
                motherModel.getAllMothers().stream()
                        .map(motherDto -> {
    ...
    public void btnAddFatherOnAction(ActionEvent actionEvent) {
        ...
        String FatherAddress = txtMotherAddress.getText();
        ...
        try {
            boolean isAdd = fatherModel.addFather(fatherDto);
            fatherReset();
            if (isAdd) {
    ...
    public void loadFatherTableData() throws SQLException, ClassNotFoundException {
        tableFather.setItems(FXCollections.observableArrayList(
                fatherModel.getAllFathers().stream()
                        .map(fatherDto -> {
                            // Create Edit button with ✏️ emoji
                            Button editButton = new Button("\uD83D\uDD8B");
                            editButton.setStyle("-fx-background-color: #4CAF50; -fx-text-fill: white;");
    ...
    public void fatherReset() {
        ...
        try {
            loadFatherTableData();
            ArrayList<String> fathers= fatherModel.getAllFatherId();
    ...
    public void editFatherDataLoader(FatherDto fatherDto) {
        lableId.setText(Integer.toString(fatherDto.getFather_id()));
        txtMotherName.setText(fatherDto.getName());
        txtMotherAddress.setText(fatherDto.getAddress());
    ...
    public void btnUpdateFatherOnAction(ActionEvent actionEvent) {
        ...
        int fatherId = Integer.parseInt(lableId.getText());
    ...
    public void btnChildAddOnAction(ActionEvent actionEvent) {
        ...
        Integer birtHeight = Integer.parseInt(txtChildHeight.getText());
        Integer birtWeight = Integer.parseInt(txtChildWeight.getText());
        ...
        try {
            boolean isUpdated = childModel.addChild(childDto);
            if (isUpdated) {
               resetChild();
    ...
    public void editChildDataLoader(ChildDto childDto) {
        ...
        lableId.setText(Integer.toString(childDto.getChild_id()));
    ...
    public void btnChildUpdateOnAction(ActionEvent actionEvent) {
        ...
        String note = txtChildNote.getText();
        String bloodType = String.valueOf(cmbMotherBloodType.getValue());

        ChildDto childDto = new ChildDto(name,address,birthPlace,gender,dob,birtHeight,birtWeight,note,bloodType);
        try {
            boolean isAdded = childModel.addChild(childDto);
            if (isAdded) {
                resetChild();
    ...
    public void loadChildTableData() throws SQLException, ClassNotFoundException {
        tableChild.setItems(FXCollections.observableArrayList(
                childModel.getAllChildren().stream()
                        .map(childDto -> {
                            // Create Edit button with ✏️ emoji
                            Button editButton = new Button("\uD83D\uDD8B");
                            editButton.setStyle("-fx-background-color: #4CAF50; -fx-text-fill: white;");
